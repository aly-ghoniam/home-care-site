...
export function TestimonialsRow({ testimonials, hasRealTestimonials = false }: TestimonialsRowProps) {
  return (
    <section aria-labelledby="testimonials-heading" className="py-24 bg-blue-pale">
      <Container>
        <motion.div
          className="mb-14"
        ...
        <ul className="grid grid-cols-1 md:grid-cols-3 gap-6" role="list">
          {testimonials.map((t, i) => (
            ...
            >
              <blockquote className="flex flex-col gap-5 rounded-2xl bg-white border border-blue-baby/50 p-8 h-full shadow-sm hover:shadow-md transition-shadow duration-200">
                <Quotes size={36} weight="fill" className="text-navy/18" aria-hidden="true" />
                <p className="text-lg text-ink/80 leading-relaxed flex-1 italic">"{t.quote}"</p>
